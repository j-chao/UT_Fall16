...
# Query functions on json_queries.sql
def count_tweets_liberia():
    stmt = "SELECT count(*) as 'Number of Tweets on Liberia' from Tweet where country_name = 'liberia';"
    run_insert(stmt)
...
def orderby_followers():
    stmt = "SELECT tweet_id, screen_name, followers_count from Tweet order by followers_count desc;"
    run_insert(stmt)

def tweets_from_Nepal():
    stmt = "SELECT tweet_id, origin, Tweet.country_name, urban_pop from Tweet left outer join Country on Country.country_name=Tweet.country_name where origin = 'Nepal';"
    run_insert(stmt)
